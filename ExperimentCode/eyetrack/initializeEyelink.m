% function el = initializeEyelink(eyefilename) 
...
%
% Inputs: 
% - global task and scr structures
...
function [el, status] = initializeEyelink(eyefilename)
...
if ~EyelinkInit(task.EYE<1);  %if task.EYE<0, then initialize in dummy mode                      % Initialization of Eyelink
...
if task.squareCalib
    ...
    calibAreaH   = task.calibShrink/aspectRatio; %makes it square
else
    calibAreaH   = task.calibShrink;
end
calibAreaV   = task.calibShrink;
